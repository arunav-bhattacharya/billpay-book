...
| **Two APIs, one library** | The DSL for explicit queries, and a lightweight DAO layer if entity mapping ever earns its keep. In practice we stay on the DSL, because payment code wants explicit reads and writes, not lazy-loaded object graphs. |
...
- **jOOQ** was the closest competitor and arguably has stronger type safety. Exposed won because it is Kotlin-native where jOOQ is Java-first. Day to day, that is the difference between the ORM disappearing and the ORM being ceremony.
- **MyBatis** brings XML mapping friction we would pay on every table.
- **Raw JDBC** is how every team ends up writing 60% of an ORM, badly.

## How we use it

There is one `Table` object per Oracle table: `TransDtl`, `TransLfcycEvent`, `IdempotencyChecker`, and friends. Those objects are the *only* types in the codebase that name columns. Activities open a `transaction { }` block, do their reads and writes, and return **immutable Kotlin data classes**, never live entities.
...
:::tip
Prefer bulk operations such as `batchInsert` or a single `update` with a where-clause over `selectAll().forEach { … update … }`. The DSL makes the row-by-row version easy to write and easy to regret.
:::
