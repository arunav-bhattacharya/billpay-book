...
HikariCP was the runner-up. It is excellent and battle-tested, and the decision was close. Agroal won on richer instrumentation and cleaner integration with our metrics stack, not because Hikari is deficient. C3P0 and the DBCP family lost on age and noisy behaviour under load. Running with no pool at all was never an option, given Oracle connection costs.
