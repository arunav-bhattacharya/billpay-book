...
- [Code layout](./code-layout.md) covers the Gradle monorepo, where each component lives, and the dependency rule the build enforces.
- [Workflows](./workflows.md) covers deterministic orchestration: one workflow per journey, composed from stages, with the spec's worked example.
- [Stages](./stages.md) covers the state transitions, one each, typed by the domain model.
