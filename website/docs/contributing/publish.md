...
- **Pushing a `gh-pages` branch does nothing.** The live site serves the workflow's artifact, not a branch, so a hand-built `gh-pages` push is silently ignored.
...
The site serves hashed bundles, so after a deploy give the CDN a minute and hard-refresh. To verify a specific change landed, check the run is green (`gh run list`) and load the page you changed. If it still looks stale, the CDN is catching up rather than the deploy failing.
